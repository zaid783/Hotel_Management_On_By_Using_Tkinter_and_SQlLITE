...
class Toplevel1:
    ...
    def place_order(self):
        ...
        c.execute("SELECT 1 FROM data WHERE Room=?", (room,))
        room_exists = c.fetchone()

        if not room_exists:
            messagebox.showerror("Error", "Invalid Room ID. Please enter a valid Room ID.")
        else:
            # Proceed with the existing logic to check food variety and quantity
            c.execute("""SELECT * FROM food WHERE FoodVariety=? AND Quantity=?""", (cusine, meal))
            result = c.fetchall()
            ...
            if result:
                ...
                cnx.commit()
                c.close()
                messagebox.showinfo("Successful", "Thank you for ordering a meal. Enjoy your meal.")
            else:
                messagebox.showerror("Error", "Food is not prepared yet.")
